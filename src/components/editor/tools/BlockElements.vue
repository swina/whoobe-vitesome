    ...
    <div v-for="group in editor.elements" :key="group.label" class="flex flex-col w-full  cursor-pointer">
        ...
        <div v-if="tab===group.label" class="flex flex-wrap items-center justify-center ">
            <template v-for="element in group.elements" :key="element.label">
                <div  class="editor-element bg-black m-1 hover:bg-purple-600 flex flex-col items-center h-16 w-16 ml-01 text-xs justify-center text-center  text-gray-300 rounded hover:text-white shadow" @click="newElement(element.name)">
        ...
        <div class="titleBar cursor-pointer"  :class="tab==='snippets'?'bg-blue-400':''" @click="tab='snippets'">Snippets</div>
