<template>
    <div v-if="open && show" :class="isShowed" class="pt-2 md:w-1/4 lg:w-1/5 text-xs cursor-pointer fixed top-0 left-0 ml-10  fixed h-screen z-modal bg-bluegray-800 text-gray-300 overflow-y-auto">
        <ul>
            <TreeItem v-if="fileTree && fileTree?.folders" :open="true" :model="fileTree.folders" @openTemplate="loadFile" @reloadTree="reload"/>
        </ul>
    </div>
</template>
